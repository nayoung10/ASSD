...
class IterativeRefinementGenerator(object):
    ...
    def generate(self, model, batch, alphabet=None, 
                 max_iter=None, strategy=None, cdr_type=None, temperature=None, replace_visible_tokens=False, 
                 need_attn_weights=False):
        alphabet = alphabet or self.alphabet
        max_iter = max_iter or self.max_iter
        strategy = strategy or self.strategy
        temperature = temperature or self.temperature
        cdr_mask = batch['cdrs'] == cdr_type # (batch_size, seq_len)


        # 1) initialized from all mask tokens
        initial_output_tokens, initial_output_scores = model.initialize_output_tokens(batch)
        prev_decoder_out = dict(
            output_tokens=initial_output_tokens,
            output_scores=initial_output_scores,
            output_masks=cdr_mask,
            attentions=None,
            step=0,
            max_step=max_iter,
        ...
        for step in range(max_iter):
            ...
            if (
                ...
                skeptical_mask = _skeptical_unmasking(
                    ...
                    output_masks=cdr_mask,  # & coord_mask,
                    p=1 - (step + 1) / max_iter
                )
                ...
                prev_decoder_out.update(output_masks=skeptical_mask)
