...
def extract_seq_from_fasta(file_path):
    """
    Parses a FASTA file given its path.
    ...
    entries = content.strip().split('>')[1:]  # Split by '>', ignore the empty string before the first '>'
    fasta_dict = {}
    
    for entry in entries:
        lines = entry.strip().split('\n')
        
        # Extract the name and split at '|' character, taking the first portion and stripping whitespace
...
class EquiAACDataset(torch.utils.data.Dataset):
    def __init__(self, file_path, fasta_path, save_dir=None, num_entry_per_file=-1, random=False, ctx_cutoff=8.0, interface_cutoff=12.0):
        '''
    ...
    def __getitem__(self, idx):
        ...
        for chains, box in zip(chain_lists, begins):
            ...
            S0.append(VOCAB.symbol_to_idx(box))
            start = len(X)
            for chain in chains:
                if chain == hc: assert len(H0) == len(chain), f'{len(H0)} != {len(chain)}, PDB ID: {item.pdb_id}' 
                for i in range(len(chain)):  # some chains do not participate
                    ...
                    if chain == hc: 
                        S0.append(VOCAB.symbol_to_idx(H0[i]))
                    else:
                        S0.append(VOCAB.symbol_to_idx(residue.get_symbol()))
        ...
        res = {
            ...
            'S0': torch.tensor(S0, dtype=torch.long),  # 1d sequence     [n_node]
    ...
    def collate_fn(cls, batch):
        ...
        return {
            ...
            'S0': torch.cat(S0s, dim=0),  # [n_all_node]
            'L': Ls,
...
def process_dataset(dataset_name):
    ...
    fasta_path = os.path.join(args.data_dir, f"{dataset_name}_pred.fasta")
    
    dataset = EquiAACDataset(dataset_path, fasta_path)
