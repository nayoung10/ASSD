...
class EfficientMCAttModel(MCAttModel):
    ...
    def forward(self, X, S, S0, L, offsets):
        '''
        :param X: [n_all_node, n_channel, 3]
        :param S: [n_all_node]
        :param S0: [n_all_node]
    ...
    def infer(self, batch, device, greedy=True):
        X, S, S0, L, offsets = batch['X'].to(device), batch['S'].to(device), batch['S0'].to(device), batch['L'], batch['offsets'].to(device)
