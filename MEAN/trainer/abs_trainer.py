...
class TrainConfig:
    def __init__(self, args, save_dir, lr, max_epoch, metric_min_better=True, early_stop=False, patience=10, grad_clip=None, anneal_base=1):
...
class Trainer:
    ...
    def _valid_epoch(self, device):
        ...
        # Determine if this metric is the best so far
        is_best = self._metric_better_best(valid_metric)
        ...
        if self._metric_better(valid_metric):
            ...
            if self._is_main_proc():
                ...
                last_save_path = os.path.join(self.model_dir, 'last.ckpt')
                torch.save(module_to_save, last_save_path)
                print_log(f'last checkpoint saved to {last_save_path}')
    ...
    def _metric_better_best(self, new):
        if self.config.metric_min_better:
            better = new < self.best_valid_metric
        else:
            better = self.best_valid_metric < new
        
        # Update the best metric if the new metric is better
        if better:
            self.best_valid_metric = new
        return better
