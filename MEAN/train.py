...
def parse():
    ...
    parser.add_argument('--train_fasta', type=str, required=True, help='path to train fasta')
    parser.add_argument('--valid_fasta', type=str, required=True, help='path to valid fasta')
    ...
    parser.add_argument('--early_stop', default=False, help='Whether to use early stop')
...
def prepare_efficient_mc_att(args):
    ...
    train_set = EquiAACDataset(args.train_set, args.train_fasta)
    train_set.mode = args.mode
    valid_set = EquiAACDataset(args.valid_set, args.valid_fasta)
